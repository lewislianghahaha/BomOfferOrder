...
namespace BomOfferOrder.UI
{
    public partial class ShowDetailFrm : Form
    {
        ...
        private int CheckRowsChangeBgc(int fmaterialid)
        {
            ...
            {
                //判断若‘物料名称’不为‘拉盖’(且‘损耗’) 并且‘标准单价’为0,就将mark=0 注:若mark为0即跳出循环
        ...
        private void Tmdel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(clickMessage, $"提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    ...
                    //注:使用‘物料编码ID’ 与 entryid 或 temprowid进行循环对比
                    for (var i = Dtl.Rows.Count; i > 0; i--)
                    {
                        for (var j = 0; j < gvdtl.SelectedRows.Count; j++)
                        {
                            //获取所选中行的materialid
                            var materialid = Convert.ToInt32(gvdtl.SelectedRows[j].Cells[1].Value);
                            //获取entryid值
                            var entryid = gvdtl.SelectedRows[j].Cells[0].Value == DBNull.Value ? 0 : Convert.ToInt32(gvdtl.SelectedRows[j].Cells[0].Value);
                            //获取temprowid值
                            var temprowid = gvdtl.SelectedRows[j].Cells[9].Value == DBNull.Value ? 0 : Convert.ToInt32(gvdtl.SelectedRows[j].Cells[9].Value);

                            //根据entryid temprowid定义checkid值
                            var checkid = entryid != 0 ? entryid : temprowid;
                            //根据entryid temprowid定义colid值
                            var colid = entryid != 0 ? 0 : 9;

                            // 若该行的colid没有值,即跳过(重)
                            if (Dtl.Rows[i - 1][colid] == DBNull.Value) continue;
                            //循环比较并进行删除
                            if (Convert.ToInt32(Dtl.Rows[i - 1][1]) == materialid && Convert.ToInt32(Dtl.Rows[i - 1][colid]) == checkid)
                            {
        ...
        private void TmHReplace_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UpdateRecordToGridView(temprowid, entryid,materialId, "HU", null);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, $"错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                               ...
                               DataTable pricelistdt,DataTable purchaseInstockdt)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        //若dt.rows[26](EntryId)为空的话,就执行
                        if (dt.Rows[0][26] == DBNull.Value)
                        {
                            FunStateEmptyOrder(dt);
                        }
                        else
                        {
                            //若dt.rows[26](EntryId)不为空的话,就表示为读取数据
        ...
        {
            //将相关值插入至对应的文本框内
            _headid = Convert.ToInt32(sourcedt.Rows[0][10]);                                                  //Headid
            txtname.Text = Convert.ToString(sourcedt.Rows[0][11]);                                           //产品名称
            txtbao.Text = Convert.ToString(sourcedt.Rows[0][12]);                                            //包装规格

            txtmi.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][13]), 4));            //产品密度(KG/L)
            txtbaochenben.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][15]), 4));    //包装成本
            txtren.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][16]), 4));           //人工及制造费用
            txtkg.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][17]), 4));            //成本(元/KG)
            txtl.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][18]), 4));             //成本(元/L)
            txt50.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][19]), 4));            //50%报价
            txt45.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][20]), 4));            //45%报价
            txt40.Text = Convert.ToString(Math.Round(Convert.ToDecimal(sourcedt.Rows[0][21]), 4));            //40%报价

            txtremark.Text = Convert.ToString(sourcedt.Rows[0][22]);                                         //备注
            txtbom.Text = Convert.ToString(sourcedt.Rows[0][23]);                                            //对应BOM版本编号
            txtcust.Text = Convert.ToString(sourcedt.Rows[0][25]);                                           //客户名称
        ...
        private DataTable GetGridViewdt(string funState,DataTable sourcedt,DataTable resultdt)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var newrow = resultdt.NewRow();
                        newrow[0] = rows[26];       //EntryId
                        newrow[1] = rows[27];       //物料编码ID
                        newrow[2] = rows[28];       //物料编码
                        newrow[3] = rows[29];       //物料名称
                        newrow[4] = rows[30];       //配方用量
                        newrow[5] = rows[31];       //占比
        ...
        private void InsertDtToGridView(DataTable sourcedt)
        {
            try
            {
                ...
                foreach (DataRow rows in sourcedt.Rows)
                {
                    //判断若获取过来的物料ID已在GridView内存在,即跳过不作添加 change date:20200616 将此限制取消(即相同的物料ID可在同一个GridView内存在多个)
        ...
        private void UpdateDtToGridView(int rowid,int entryid,int materialId, DataTable sourcedt)
        {
            try
            {
                //根据entryid 及 rowid 情况赋值给checkid
                var checkid = entryid != 0 ? entryid : rowid;
                //根据entryid 及 rowid 情况赋值给id(行下标ID)
                var colid = entryid != 0 ? 0 : 9;
                ...
                {
                    // 若该行的colid没有值,即跳过
                    if (rows[colid] == DBNull.Value) continue;
                    if (Convert.ToInt32(rows[colid]) == checkid && Convert.ToInt32(rows[1]) == materialId)
                    {
